...
跑 `samples/devices/recording_missing_cases.json` 中的固定子案例，
输出每个案例的候选根因、引用合规情况和总体统计：

    demo-output/phase2-recording-missing-eval.json
    demo-output/phase2-recording-missing-eval.md
...
JSON_PATH = OUTPUT_DIR / "phase2-recording-missing-eval.json"
MD_PATH = OUTPUT_DIR / "phase2-recording-missing-eval.md"
CASE_REPORT_DIR = OUTPUT_DIR / "phase2-cases"

REVIEWER = "eval-harness"
DESCRIPTION = "目标时间段录像缺失，需要定位根因"

# 样例数据中必须被脱敏的占位凭证，用于统计敏感信息泄露。
SENSITIVE_MARKERS: tuple[str, ...] = (
    "sample-admin-pwd-not-real",
    "sample-token-not-real",
)
...
def check_citation_compliance(case, conclusion) -> bool:
    ...
    - 有结论且至少引用一条 Evidence；
    - 所有引用都属于当前诊断；
    - probable 至少引用两类设备事实 Evidence（Phase 2C 起含录像类）；
    - 不允许出现 confirmed 结论。
    """
...
def render_markdown(payload: dict) -> str:
    ...
    lines += [
        ...
        "- 录像类证据（录像计划 / 存储状态 / 回放检查）已纳入 CitationPolicy 设备事实类型。",
        "",
    ]
    return "\n".join(lines)
