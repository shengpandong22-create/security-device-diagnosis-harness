"""Phase 4C 报警误报固定案例评测脚本。

运行：

    uv run python scripts/eval_phase4_alarm_false_positive.py

跑 `samples/devices/alarm_false_positive_cases.json` 中的固定子案例，
输出每个案例的候选根因、引用合规情况和总体统计：

    demo-output/phase4-alarm-false-positive-eval.json
    demo-output/phase4-alarm-false-positive-eval.md
...
JSON_PATH = OUTPUT_DIR / "phase4-alarm-false-positive-eval.json"
MD_PATH = OUTPUT_DIR / "phase4-alarm-false-positive-eval.md"
CASE_REPORT_DIR = OUTPUT_DIR / "phase4-cases"

REVIEWER = "eval-harness"
DESCRIPTION = "报警疑似误报，需要定位规则、环境、传感器、复核或重复告警根因"

# 样例数据中必须被脱敏的占位信息，用于统计敏感信息泄露。
SENSITIVE_MARKERS: tuple[str, ...] = (
    "sample-admin-pwd-not-real",
    "http://example.local/snapshot/not-real.jpg",
)
...
def run_eval() -> dict:
    """跑完全部固定案例，返回评测结果字典。"""
    container = build_phase4_container()
...
def render_markdown(payload: dict) -> str:
    ...
    lines += [
        ...
        "- 报警类证据（规则 / 信号 / 环境 / 复核 / 关联）已纳入 CitationPolicy 设备事实类型。",
        "",
    ]
    return "\n".join(lines)
