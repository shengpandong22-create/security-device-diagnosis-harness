...
SCRIPT_PATH = REPO_ROOT / "scripts" / "eval_phase4_alarm_false_positive.py"


def _load_module():
    spec = importlib.util.spec_from_file_location("eval_phase4_alarm_false_positive", SCRIPT_PATH)
...
def test_phase4_eval_runs_all_cases():
    module = _load_module()
...
def test_phase4_eval_label_accuracy_is_full():
    module = _load_module()
...
def test_phase4_eval_citation_compliance_is_full():
    module = _load_module()
...
def test_phase4_eval_does_not_call_external_model():
    module = _load_module()
...
def test_phase4_eval_does_not_leak_sensitive_values():
    module = _load_module()
...
def test_phase4_eval_case_fields_are_complete():
    module = _load_module()
...
def test_phase4_eval_markdown_report_contains_summary():
    module = _load_module()
    payload = module.run_eval()

    markdown = module.render_markdown(payload)

    assert "Phase 4C 报警误报评测报告" in markdown
    ...
    assert "rule_too_sensitive" in markdown
