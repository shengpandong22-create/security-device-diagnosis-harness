...
- [x] 新案例经过脱敏、来源、双人标注、裁决和泄漏检查
- [x] 发布新语义版本且旧版本不可变
- [x] Validation/Test 继续物理隔离
- [x] 真实模型仍为显式授权、单次调用、无自动重试
- [x] 报告明确区分合成样本覆盖与真实业务覆盖

Phase 8D 已完成：发布输入必须携带 DatasetCase、盲标任务、两份不同标注员意见、独立裁决、裁决生成的准入候选和来源授权谱系；发布边界重新执行所有验证，不信任单独提交的候选对象。新增案例会与全部旧案例及同批新增案例执行身份、来源、结构指纹和同域近重复检查。新版本先在 staging 目录生成并完成 Manifest/hash/三 split 隔离验证，全部通过后才原子重命名为不可变语义版本；已存在目标目录拒绝覆盖。
...
该脚本临时发布 `1.1.0` 协议候选：保留原 6 个案例并新增 1 个明确标识的合成 Dev 案例。发布回执分别统计 synthetic 与 authorized 数量，不将该样本描述为真实业务覆盖；脚本不读取密封 Test 内容，不调用真实模型、BGE 或设备。

## 5. 总体验收

- [x] 能证明“标准答案如何产生”，而不只是展示准确率数字
- [x] 能解释一次退化属于数据、工具、证据、模型还是基础设施
- [x] 不通过修改标准答案迎合模型
- [x] 不把小样本满分描述成生产准确率
