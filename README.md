...
- 当前阶段：Phase 0～8 已完成；已形成诊断、知识、持久化和评测运营闭环。
...
Phase 8C 只持久化脱敏运行身份、聚合指标、Baseline 血缘和 Gate 结论。不同数据集、模型、Prompt、配置、Runner 或环境的结果不能混入同一趋势；候选退化继续复用 Phase 7 确定性发布门禁。

Phase 8D 将 DatasetCase、双人盲标、独立裁决、来源授权和近重复检查串成不可绕过的发布协议。新版本只在 staging 全量验证通过后可见，已有语义版本禁止覆盖；发布回执明确区分合成案例与授权业务案例。固定脚本只发布临时 `1.1.0` 协议候选，仓库当前正式数据集仍为 `1.0.0`。
